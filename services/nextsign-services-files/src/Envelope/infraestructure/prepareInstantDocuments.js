...
} = require("../middleware/getDocumentFileByType");
const { DocumentType } = require("../../helpers/documentType");
const { uploadFile } = require("../../utils/uploadFile");
const sendRes = require("../../helpers/send.res");
const {
  getInstantDocumentFileByType,
} = require("../../services/instantSignment/getInstantDocumentFileByType");
...
function uploadSignedFile(signment, document) {
  const date = moment(signment.created_at.replace(" ", ""), "YYYY-MM-DD");
  const YEAR = moment(date).format("YYYY");
  const MONTH = moment(date).format("MM");
  const DAY = moment(date).format("DD");
  const path_to_save = `${YEAR}/${MONTH}/${DAY}/${signment.id}/${document.id}`;
  const filename = `${document.file_name.split(".")[0]}-SIGNED.pdf`;
  return { filename, path_to_save };
}

async function prepareInstantDocuments(req, res) {
  try {
    const body = req.body;
    console.log(body);
    const { instant_signment } = req.body;

    let updatedDocuments = 0;
    const { id } = instant_signment;
    const prisma = new PrismaClient();

    const documents = await prisma.document.findMany({
      where: {
        instant_signment_id: parseInt(instant_signment.id),
        deleted_at: null,
      },
    });
    for (let index = 0; index < documents.length; index++) {
      ...
      const file = await getInstantDocumentFileByType(
        parseInt(id),
        parseInt(idDocument),
      ...
      if (tags.length < 1) {
        let newTags = [];
        if (tags.length === 0) {
          newTags = generateTags(
            recipients,
            idDocument,
            null,
            parseInt(pages_document) + 1
          );
          numero_hojas = Math.ceil(recipients.length / 20); // Generate pages
        } else {
          const tags_recipients = [...tags.map((tag) => tag.recipient_id)];
          const recipient_pending = recipients.filter(
            (src) => !tags_recipients.includes(src.id)
          );
          const recipient_with_tag =
            recipients.length - recipient_pending.length;
          ...
          if (numero_hojas_agregadas == numero_hojas) {
            ...
            init_page_from =
              parseInt(pages_document) + parseInt(numero_hojas_agregadas);
          }
      ...
      if (numero_hojas > 0) {
        Array.from({ length: numero_hojas }, () =>
          pdfDoc.addPage(PageSizes.A4)
        );
        attachment = await pdfDoc.saveAsBase64({ dataUri: true });
      }
      const { filename, path_to_save } = uploadSignedFile(
        instant_signment,
        document
      );
      const update = await addDocumentUpdated(
        { attachment, filename, path_to_save },
        document
      );
      console.log(update);
...
function generateTags(
  ...
  user,
  init_page = 0,
  init_firmas_por_columna = 0,
  init_firmas_por_fila = 0
) {
  let tags = [];
  const firmas_por_hojas = 20;
  const firmas_por_fila = 2;
  const firmas_por_columna = firmas_por_hojas / firmas_por_fila;
  const numero_hojas = Math.ceil(signment.length / firmas_por_hojas);
  ...
  const total_tags = signment.length;
  let init = init_page;
  let current_tag = 0;
    ...
    for (let y = init_firmas_por_columna; y < firmas_por_columna; y++) {
      for (let x = init_firmas_por_fila; x < firmas_por_fila; x++) {
        if (current_tag < total_tags) {
          const recipient_id = signment[current_tag].id;
          const instant_signment_id = signment[current_tag].instant_signment_id
          const tag = {
            position_x1: parseInt(
              separacion_ancho + (ancho_firma + separacion_ancho) * x
            ),
            position_y1: parseInt(
              separacion_alto + (alto_firma + separacion_alto) * y
            ),
            position_x2: parseInt(
              separacion_ancho +
                (ancho_firma + separacion_ancho) * x +
                ancho_firma
            ),
            position_y2: parseInt(
              separacion_alto + (alto_firma + separacion_alto) * y + alto_firma
            ),
            page: init,
            instant_signment_id,
            recipient_id,
...
module.exports = { prepareInstantDocuments };
